...
class TransactionService:
    @staticmethod
    def create_borrow_request(user, equipment_uuid, due_date=None, reason='', image=None):
        """
    ...
    def create_return_request(user, equipment_uuid):
        ...
        with transaction.atomic():
            ...
            latest_borrow = equipment.transactions.filter(
                action=Transaction.Action.BORROW,
                status=Transaction.Status.COMPLETED
            ).order_by('-created_at').first()

            is_privileged = user.role in [User.Role.MANAGER, User.Role.ADMIN] or user.is_staff
    ...
    @staticmethod
    def approve_transaction(admin_user, transaction_id, admin_note='', new_location_data=None):
        """
        ...
        with transaction.atomic():
            ...
            if txn.action == Transaction.Action.BORROW:
                ...
                if new_location_data:
                    ...
                    if 'zone' in new_location_data: equipment.zone = new_location_data['zone']
                    if 'cabinet' in new_location_data: equipment.cabinet = new_location_data['cabinet']
                    if 'number' in new_location_data: equipment.number = new_location_data['number']
