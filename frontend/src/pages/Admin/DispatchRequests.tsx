...
export const DispatchRequests = () => { 
  ...
  const approveMutation = useMutation({
    mutationFn: ({ id, admin_note }: { id: number, admin_note?: string }) => 
        transactionsApi.approveDispatch(id, admin_note),
    onSuccess: () => {
      alert('已批准出庫');
  ...
  const rejectMutation = useMutation({
    mutationFn: (id: number) => transactionsApi.rejectDispatch(id, 'Admin rejected'),
    onSuccess: () => {
      alert('已拒絕出庫');
      queryClient.invalidateQueries({ queryKey: ['transactions'] });
      queryClient.invalidateQueries({ queryKey: ['equipment'] });
      ...
      <header className="bg-white shadow-sm px-6 py-4 flex items-center justify-between sticky top-0 z-20 shrink-0">
        <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
                ...
                <h1 className="text-xl font-black tracking-tight">出庫申請審核</h1>
            </div>
        </div>
        ...
        <div className="flex bg-gray-100 p-1 rounded-2xl shadow-inner border border-gray-200">
            <button 
                ...
                className={`px-6 py-2 rounded-xl text-sm font-black transition-all ${filter === 'PENDING_APPROVAL' ? 'bg-white text-orange-600 shadow-md' : 'text-gray-500 hover:text-gray-700'}`}
            >
                待審核
            </button>
            <button 
                onClick={() => setFilter('ALL')}
                className={`px-6 py-2 rounded-xl text-sm font-black transition-all ${filter === 'ALL' ? 'bg-white text-orange-600 shadow-md' : 'text-gray-500 hover:text-gray-700'}`}
            >
                所有記錄
            </button>
        </div>
                ...
                <table className="w-full border-collapse">
                    <thead>
                        <tr className="bg-gray-50/50 border-b border-gray-100 text-xs font-black text-gray-700 uppercase tracking-widest">
                            ...
                            <th className="px-6 py-4 text-left">原因 / 目的地</th>
                    ...
                    <tbody className="divide-y divide-gray-50">
                        {isLoading ? (
                            ...
                            transactions.map((txn) => (
                                <tr key={txn.id} className={`hover:bg-gray-50/50 transition-colors group ${selectedIds.includes(txn.id) ? 'bg-orange-50/50' : ''}`}>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="space-y-1">
                                            ...
                                            {txn.admin_note && (
                                                <div className="flex items-start gap-2 text-orange-600 font-bold max-w-xs">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        {txn.status === 'PENDING_APPROVAL' ? (
                                            ...
                                            <span className="px-3 py-1 rounded-full text-xs font-black bg-green-100 text-green-700 flex items-center gap-1 w-fit">
                                                <CheckCircle className="w-3 h-3" /> 已出庫
                                            </span>
                                        ) : (
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right">
                                        {txn.status === 'PENDING_APPROVAL' ? (
                                            <div className="flex justify-end gap-2">
                                                <button
                                                    onClick={() => {
                                                        const note = prompt(`確認批准此出庫申請？\n您可以輸入核准備註(如物流單號):`);
                                                        if(note !== null) {
                                                            approveMutation.mutate({ id: txn.id, admin_note: note });
                                                        }
                                                    }}
                                                    disabled={approveMutation.isPending}
                                                    className="flex items-center gap-1 bg-orange-600 text-white px-3 py-1.5 rounded-lg text-xs font-black hover:bg-orange-700 transition-all shadow-md shadow-orange-100"
                                                >
                            ...
                            <tr><td colSpan={7} className="px-6 py-20 text-center text-gray-400 font-bold">目前沒有{filter === 'PENDING_APPROVAL' ? '待審核' : ''}的出庫申請</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
      ...
      {selectedIds.length > 0 && (
          <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-white px-6 py-4 rounded-3xl shadow-2xl border border-gray-100 flex items-center gap-6 z-50 animate-in fade-in slide-in-from-bottom-4 duration-300">
              ...
              <div className="flex gap-3">
                  <button 
                    onClick={() => {
                        const note = prompt(`您即將核准 ${selectedIds.length} 筆出庫申請。\n您可以輸入統一的核准備註(如物流單號):`);
                        if (note !== null) {
                            bulkApproveMutation.mutate({ ids: selectedIds, admin_note: note });
                        }
                    }}
                    disabled={bulkApproveMutation.isPending}
                    className="flex items-center gap-2 bg-orange-600 text-white px-6 py-2.5 rounded-2xl text-sm font-black hover:bg-orange-700 transition-all shadow-lg shadow-orange-200"
                  >
