...
透過整合 **QR Code 快速掃描**技術，使用者僅需使用手機或 WebCam 掃描設備上的標籤，即可快速檢視設備詳情、狀態或進行借還操作。

> **現代化的資產管理解決方案，讓設備追蹤變得輕而易舉。**

系統內建完整的**權限管理**（Admin/Manager/User）與**簽核流程**，並採用 **Docker** 全容器化部署，讓管理者無需繁雜的設定即可輕鬆掌握資產動態。
...
1.  **學術實驗室**：精密儀器、耗材與借用人追蹤。
2.  **新創辦公室**：筆電、測試機、周邊配件管理。
3.  **創客空間**：手工具、3D 列印機等共享設備租借。
4.  **企業資產盤點**：快速產出 QR Code 標籤，簡化年度盤點流程。
...
| 功能模組 | 說明 |
| :--- | :--- |
| **儀表板 (Dashboard)** | 統計目前的資產總數、借出率及待審核事項。 |
| **掃描中心 (Scan Center)** | 快速辨識設備標籤，即時跳轉至操作頁面。 |
| **資產清單 (Equipment List)** | 強大的篩選功能，支援按狀態、分類或位置快速搜尋。 |
| **層級管理 (Location Tree)** | 樹狀結構展示辦公室/倉庫佈局，支援拖放移動。 |
...
*   **[✨ 系統演示 (Demo Script)](docs/DEMO.md)**
    *   演示劇本，包含系統概覽、資產管理、借用旅程與審核流程。
